...
export class ServicesPage implements AfterViewInit, OnDestroy {
      ...
      Flip.from(state, {
        ...
        onEnter: (els: Element[]) =>
          gsap.fromTo(
            ...
            { opacity: 1, scale: 1, duration: 0.4, delay: 0.08 },
          ),
        onLeave: (els: Element[]) => gsap.to(els, { opacity: 0, scale: 0.85, duration: 0.28 }),
        onComplete: () => {
  ...
  private initWorkGrid(): void {
    ...
    allItems.forEach((el, i) => {
      if (i >= this._allMax) el.style.display = 'none';
    });
  ...
  private initCase(): void {
    ...
    slides.forEach((_s, i) => {
      ...
      tl.to(
        slides[i],
        { yPercent: 0, scale: 1, opacity: 1, rotationX: 0, duration: 1, ease: 'power2.inOut' },
        i - 1,
      );
      tl.to(
        slides[i - 1],
        { yPercent: -12, scale: 0.92, opacity: 0, rotationX: 8, duration: 1, ease: 'power2.inOut' },
        i - 1,
      );
    });
